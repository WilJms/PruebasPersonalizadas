"""Write the semantic-benchmark/1.3.2 pre-execution package.

1.3.2 rebinds the U3 semantic qualification claim to the benchmark version it
actually applies to, and changes nothing else.  ``semantic-benchmark/1.3.1`` is
not edited; no provider or adjudicator ever ran against it, so superseding it is
an authority-binding repair rather than result-driven tuning.

Six artifacts are republished byte-identically from 1.3.1 -- the N3 provider
fixtures, the N3 axis, the call budget and the three N3 proofs.  Their material
did not change, so their hashes must not: they are copied rather than
regenerated with a new version stamp, and the lineage and the stale-claim scan
both record that they are republished.
...
def main() -> int:
    build = build_v132(DEFAULT_CORPUS_ROOT)
    package = v132_package(build)

    if serialize(package) != serialize(v132_package(build)):  # pragma: no cover
        print("V132_PACKAGE_NOT_DETERMINISTIC_WITHIN_A_BUILD")
        return 1
    ...
    if drifted:
        print(f"V132_PACKAGE_NOT_DETERMINISTIC_ACROSS_BUILDS: {drifted}")
        return 1
    ...
    if unregistered or orphaned:
        print(
            "V132_SELF_HASH_REGISTRY_OUT_OF_SYNC: "
    ...
    artifacts = [
        {
            ...
            "republished_unchanged_from": (
                "semantic-benchmark/1.3.1" if relative in REPUBLISHED_FROM_V131 else None
            ),
    ...
    manifest_material = {
        "schema_version": "phase9-freeze-hash-manifest/1.3.2",
        ...
        "supersedes": "phase9-freeze-hash-manifest/1.3.1",
    ...
    print("SEMANTIC_BENCHMARK_V1_3_2_PREEXECUTION_FREEZE_READY_FOR_FRESH_AUDIT")
    print(f"benchmark_version              {SEMANTIC_BENCHMARK_V132_VERSION}")
